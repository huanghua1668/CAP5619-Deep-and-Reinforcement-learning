...
def preprocess(seqs0, maxLen=100):
    # have to pad pre, otherwise won't be able to predict the 1st char
    ...
    for i in range(len(seqs0)):
        ...
        if sz>maxLen+1:
            ...
            for j in range(maxLen+1-sz):
                #embed_x.append(0)
                embed_x.insert(0, 0)#insert 0 at the beginning
        if i==0: 
    ...
    if not tableCalculated:
        ...
        for i in range(1, charsSize):
            ...
            prob[1]=model.predict(chars1[:, :1, :],
                           batch_size=None, verbose=0)[0][0]
            # this step only output 1 char(input 1, output 1)
            prob[1]*=prob[0][i]
            print(i, '2nd char', prob[1])
            #predict 3rd char
            for j in range(1, charsSize):
                ...
                prob[2]=model.predict(chars2[:, :2, :],
                               batch_size=None, verbose=0)[0][-1]
                # now predict output 2 chars, different j will not
                # affect the prediction for the 1st output char, will
                # only affect the 2nd output char
                prob[2]*=prob[1][j]
                if i==6:
                    print(i, j, '3rd char', prob[2])
                #predict 4th char
                for k in range(1, charsSize):
                    chars3=np.copy(chars2)
                    chars3[0, 2][k]=1
                    #if i==6 and j==7:
                    #    print('k=', k, model.predict(chars3[:, :3, :],
                    #          batch_size=None, verbose=0)[0][-1])
                    prob[3]=model.predict(chars3[:, :3, :],
                                   batch_size=None, verbose=0)[0][-1]
                    # now predict outputs 3 chars, different k will not
                    # affect the prediction for the 1st and 2nd output char, 
                    # will only affect the 3rd output char
                    prob[3]*=prob[2][k]
                    #predict 5th char
