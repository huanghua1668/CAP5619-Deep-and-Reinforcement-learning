...
class Sarsa_lambda:
    def __init__(self, stepSize, lam, dimension=6,
        ...
        self.weights['w3'] = np.random.randn(2, 3, 1, self.H)/np.sqrt(self.H)
        self.weights['b3'] = np.ones((2, 3, 1))
    ...
    def traceUpdate(self, x, h1, h2, action, goRight):
        ...
        self.traces['w3'][goRight, act, 0]+=h2
        self.traces['b3'][goRight, act, 0]+=1.

        dqdh2=self.weights['w3'][goRight, act, 0]
        dqdh2[h2<=0]=0.
    ...
    def learn(self, goRight, x, action, reward, newGoRight, newX, newAction, ep,
        ...
        h1, h2, estimation = self.forward(x, action, goRight)
        h1_, h2_, newEst = self.forward(newX, newAction, newGoRight)
        if justScored:
            newEst=0.
        delta = reward+ DISCOUNT*newEst - estimation
        self.traceUpdate(x, h1, h2, action, goRight)
        for k, v in self.traces.items():
...
def getAction(x, goRight, valueFunction):
    ...
    for action in ACTIONS:
        h1_, h2_, value = valueFunction.forward(x, action, goRight)
        values.append(value)
