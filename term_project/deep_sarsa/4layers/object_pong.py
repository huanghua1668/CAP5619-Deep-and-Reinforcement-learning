...
class Sarsa_lambda:
    def __init__(self, stepSize, lam, dimension=6,
        ...
        self.weights['w4'] = np.random.randn(2, 3, 1, self.H)/np.sqrt(self.H)
        self.weights['b4'] = np.ones((2, 3, 1))
    ...
    def traceUpdate(self, x, h1, h2, h3, action, goRight):
        ...
        self.traces['w4'][goRight, act, 0]+=h3
        self.traces['b4'][goRight, act, 0]+=1.

        dqdh3=self.weights['w4'][goRight, act, 0]
        dqdh3[h3<=0]=0.
    ...
    def learn(self, goRight, x, action, reward, newGoRight, newX, newAction, ep,
        ...
        h1, h2, h3, estimation = self.forward(x, action, goRight)
        h1_, h2_, h3_, newEst = self.forward(newX, newAction, newGoRight)
        if justScored:
            newEst=0.
        delta = reward+ DISCOUNT*newEst - estimation
        self.traceUpdate(x, h1, h2, h3, action, goRight)
        for k, v in self.traces.items():
...
def getAction(x, goRight, valueFunction):
    ...
    for action in ACTIONS:
        h1_, h2_, h3_, value = valueFunction.forward(x, action, goRight)
        values.append(value)
    ...
    for lamInd, lam in enumerate(lams):
        for alphaInd, alpha in enumerate(alphas):
            for run in range(runs):
                ...
                for ep in range(10001, episodes):
